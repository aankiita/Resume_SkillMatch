...
class JobAnalyzer:
    ...
    def analyze_job(self, job_description: str) -> dict:
        ...
        Format the response strictly as a JSON object with these categories.
        Job Description:
        {description}
        """
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[{"role": "user", "content": prompt.format(description=job_description)}],
    ...
    def analyze_resume(self, resume_text: str) -> dict:
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model_name,
                messages=[{"role": "user", "content": prompt.format(resume=resume_text)}],
                response_format={"type": "json_object"},
                temperature=0.2
            )
            parsed_response = json.loads(response.choices[0].message.content)
            ...
            st.error(f"Error parsing resume analysis response: {str(e)}. Please check the resume text for any formatting issues.")
            return {}
        except Exception as e:
...
def main():
    ...
    # Use the latest capable model on Groq for tasks requiring reasoning and JSON.
    ...
    # Initialize analyzers passing the Groq client and model
    job_analyzer = JobAnalyzer(client=client, model_name=model_name)
    cover_letter_gen = CoverLetterGenerator(client=client, model_name=model_name)
